...
remote_state_key="${ADE_ENVIRONMENT_NAME}/${ADE_ENVIRONMENT_TYPE}/deploy.tfstate"
...
# List blob containers in the specified storage account
echo -e "\n>>> Listing Blob Containers...\n"
az storage container list --auth-mode login --account-name $remote_state_sa --query "[].name" -o tsv
